...
const Summary = () => {
  ...
  useEffect(() => {
    const articlesummarizedata = JSON.parse(localStorage.getItem("article"));
    if (articlesummarizedata) {
      setAllArticles(articlesummarizedata);
    }
  }, []);
  ...
  const handleDelete = (index) => {
    const filter = allArticles.filter((item, idx) => {
      return idx !== index;
    });
    setAllArticles(filter);
    localStorage.setItem("article", JSON.stringify(filter));
  };
